...
#pragma omp target teams distribute parallel for collapse(2) reduction(max:err) schedule(nonmonotonic:static,1)
        for( j = 1; j < n-1; j++) {
            for( i = 1; i < m-1; i++ ) {
	...
	// TODO: the same here
#pragma omp target teams distribute parallel for collapse(2) schedule(nonmonotonic:static,1)
        for( j = 1; j < n-1; j++) {
            for( i = 1; i < m-1; i++ ) {
                A[j *m+ i] = Anew[j *m+ i];
            }
